...
int main(int argc, char**argv) {
  ...
  if (argc > 3) {
    ...
    if (deliverySystem) {
      deliverySystem->AddFactory(new CustomerFactory());
      deliverySystem->AddFactory(new DroneFactory());
      deliverySystem->AddFactory(new PackageFactory());
      deliverySystem->AddFactory(new RobotFactory());
    }
